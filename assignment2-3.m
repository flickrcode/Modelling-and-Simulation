...
for i=3:1:N/2 % change the index to i=N/2:-1:4 for the 3rd ARX
...
for i=3:1:NN % For the 3rd ARX model, we switch the index to i=NN:-1:4
...
%   ypred(i) = -ahat_1*yn(i-1)-ahat_2*yn(i-2) + bhat_0*un(i) +bhat_1*un(i-1);
    %candidate 3
%   ypred(i)= -ahat_1*yn(i-1)-ahat_2*yn(i-2)-ahat_3*yn(i-3) + bhat_1*un(i-1);
end
...
noise_std = 0.01;
covariance = inv(PHI*PHI.')*(noise_std^2)
